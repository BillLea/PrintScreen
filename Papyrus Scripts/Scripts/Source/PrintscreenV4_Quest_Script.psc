...
;Validate initial path on startup
 Result = PrintSCreenV4_formula_SCript.printscreenV4(Validate,Path,Imagetype,Compression )
if(Result != "Success")
...
Event OnKeyUP(int theKey, float holdtime)
  ...
  Result = PrintscreenV4_Formula_SCript.PrintScreenV4(0,Path,ImageType, Compression)
    if(Result == "Success")
       Shots = shots + 1
    Endif
